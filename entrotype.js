...
$(document).ready(function(){
  ...
  KeyboardLayout.prototype.splitSpecStr = function(specStr) {
    var split = {rows: "", hands: "", fingers: "", modifiers: ""};
    for (ci in specStr) {
      var c = specStr[ci];
      if (this._row_dict[c]) {
        split.rows += c;
  ...
  Game.prototype.showRandomChar = function(container) {
    var charnum = Math.floor(Math.random() * 256);
    console.log(String.fromCharCode(charnum));
  };

  // Global information.
  ansi_layout = new KeyboardLayout(ANSI_QWERTY);
  game = new Game();
  ...
  $("#btn_start").
    ...
    click(function(event){
      ...
      console.log("Clicked button " + target.value);
  ...
  // - Show a key to strike, listen for key events
  // - Set a timer for a maximum delay for a press to occur
  //    - Key pressed: show happiness (or sadness) then next character
  //    - Timer fired: show sadness then next character
  // - Always set a new timer with a new character
  // - Go until stopped.
});
